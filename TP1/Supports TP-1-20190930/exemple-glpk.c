...
int exemple_glpk(int argc, const char * argv[]) {
	...
	glp_set_row_bnds(lp, 1, GLP_DB, 30, 100);

	glp_set_row_name(lp, 2, "q");
	glp_set_row_bnds(lp, 2, GLP_DB, 2, 40);

	glp_set_row_name(lp, 3, "r");
	glp_set_row_bnds(lp, 3, GLP_DB, 25, 80);
  ...
  /* Coeff de la fonction a maximiser */
	glp_set_obj_dir(lp, GLP_MAX);
	glp_set_obj_coef(lp, 1, 19);
	glp_set_obj_coef(lp, 2, 6);
	glp_set_obj_coef(lp, 3, 13);
  glp_set_obj_coef(lp, 4, 52);
